...
public class DefaultSkipperClient implements SkipperClient {
	...
	@Override
	public void deleteRepository(String name) {
		ParameterizedTypeReference<Repository> typeReference =
				new ParameterizedTypeReference<Repository>() { };
		UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(baseUri);
		builder.pathSegment("repositories", "search", "findByName");
		builder.queryParam("name", name);

		ResponseEntity<Repository> resourceResponseEntity =
				restTemplate.exchange(builder.toUriString(),
						HttpMethod.GET,
						null,
						typeReference);
		if (resourceResponseEntity.getBody() != null) {
			Map<String, String> uriVariables = new HashMap<String, String>();
			uriVariables.put("id", Long.toString(resourceResponseEntity.getBody().getId()));
			this.restTemplate.delete(baseUri + "/repositories/{id}", uriVariables);
		}
		else {
			throw new IllegalStateException("The Repository with the " + name + " doesn't exist.");
		}
	...
	public Collection<Repository> listRepositories() {
		...
		ResponseEntity<HateoasResponseWrapper<RepositoriesResponseWrapper>> resourceResponseEntity =
				restTemplate.exchange(baseUri + "/repositories",
						HttpMethod.GET,
						null,
						typeReference,
						uriVariables);
	...
	public Collection<Deployer> listDeployers() {
		...
		ResponseEntity<HateoasResponseWrapper<DeployersResponseWrapper>> resourceResponseEntity =
				restTemplate.exchange(baseUri + "/deployers",
						HttpMethod.GET,
						null,
						typeReference,
						uriVariables);
