...
public interface ReleaseRepository extends KeyValueRepository<Release, Long>, ReleaseRepositoryCustom {
	...
	@Transactional(readOnly = true)
	@RestResource(exported = false)
	default Release findTopByNameOrderByVersionDesc(@Param("name") String name) {
		return findTopByNameAndInfoStatusStatusCodeNotOrderByVersionDesc(name, StatusCode.CANCELLED);
	}
	...
	@Transactional(readOnly = true)
	@RestResource(exported = false)
	Release findTopByNameAndInfoStatusStatusCodeNotOrderByVersionDesc(@Param("name") String name, @Param("statusCode") StatusCode statusCode);
